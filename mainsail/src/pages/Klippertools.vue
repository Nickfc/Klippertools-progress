<template>
  <v-container fluid class="service-manager py-0">
    ...
    <v-alert dense text type="info" class="mb-4">
      Motion is commanded Klipper travel, not encoder feedback. Recommended
      intervals are conservative starting points—inspect sooner after crashes,
      contamination, noise, looseness, or changed print quality.
    </v-alert>
    <v-alert
      v-if="pageError"
      dense
          ...
          <div class="d-flex align-center justify-space-between mb-3">
            <div>
              ...
              <div class="text-caption text--secondary">
                Tasks are sorted by urgency. Any enabled threshold can make a
                task due.
              </div>
            </div>
            <v-chip small outlined :color="dueCount ? 'error' : 'success'"
              >{{ dueCount }} due</v-chip
            >
          </div>
          <v-row dense>
              ...
              >
                  ...
                  <v-expansion-panels flat class="mt-2">
                    <v-expansion-panel>
                      <v-expansion-panel-header class="px-0 py-1 text-caption"
                        >Service instructions</v-expansion-panel-header
                      >
                ...
                <v-card-actions>
                  <v-btn small text @click="editTask(task)"
                    ...
                    >Edit</v-btn
                  >
                  <v-btn small text @click="snooze(task, '24h')"
                    >Snooze 24h</v-btn
                  >
                  <v-spacer />
        ...
        <v-tab-item>
          <v-card outlined>
            ...
            <v-list v-if="status.history.length" two-line>
              <v-list-item
                v-for="(entry, index) in status.history"
                :key="`${entry.at}-${index}`"
              >
                <v-list-item-icon
                  ><v-icon>{{
            ...
            <v-card-text v-else class="text--secondary"
              >History starts when a task is marked serviced or settings are
              reset/imported.</v-card-text
            >
          </v-card>
        </v-tab-item>
        ...
        <v-tab-item>
          <v-row dense>
            <v-col cols="12" md="6">
              <v-card outlined class="h-100">
                <v-card-title class="text-subtitle-1"
                  >Recommended schedule</v-card-title
                >
                <v-card-text
                  >Restore the ten Ender 7 starting presets and remove custom
                  tasks or interval overrides. Existing service baselines and
                  history are preserved.</v-card-text
                >
                <v-card-actions
                  ><v-btn
                    text
                    color="primary"
                    @click="showRecommendedReset = true"
                    >Reset to recommended</v-btn
                  ></v-card-actions
                >
              </v-card>
            </v-col>
            <v-col cols="12" md="6">
              <v-card outlined class="h-100">
                <v-card-title class="text-subtitle-1"
                  >Import and export</v-card-title
                >
                <v-card-text
                  >Export tasks and history as JSON, or import a previously
                  exported task schedule. Lifetime counters are never
                  imported.</v-card-text
                >
                <v-card-actions>
                  <v-btn text color="primary" @click="exportData"
                    >Export JSON</v-btn
                  >
                  <v-btn text color="primary" @click="$refs.importInput.click()"
                    >Import JSON</v-btn
                  >
                  <input
                    ref="importInput"
            ...
            <v-col cols="12">
              <v-card outlined class="danger-zone">
                <v-card-title class="text-subtitle-1 error--text"
                  >Advanced: lifetime counters</v-card-title
                >
                <v-card-text
                  >This resets every lifetime usage counter and every task
                  baseline. It is blocked while printing and service history
                  records the reset.</v-card-text
                >
                <v-card-actions
                  ><v-btn text color="error" @click="showCounterReset = true"
                    >Reset lifetime counters</v-btn
                  ></v-card-actions
                >
              </v-card>
            </v-col>
    ...
    <v-dialog v-model="showEditDialog" max-width="720" persistent>
      <v-card>
        <v-card-title>{{
          draft.id ? "Edit service task" : "Add service task"
        }}</v-card-title>
        <v-card-text>
          ...
          <div
            v-for="(threshold, index) in draft.thresholds"
            :key="index"
            class="threshold-row"
          >
            <v-select
          ...
          <v-alert v-if="editError" dense text type="error" class="mt-3 mb-0">{{
            editError
          }}</v-alert>
        </v-card-text>
        <v-card-actions>
          <v-btn v-if="draft.id" text color="error" @click="deleteDraft"
            >Delete</v-btn
          >
          <v-spacer />
    ...
    <v-dialog v-model="showRecommendedReset" max-width="520">
      <v-card
        ...
        ><v-card-text
          >Custom tasks and interval overrides will be removed. Service history
          and current task baselines are preserved.</v-card-text
        ><v-card-actions
          ><v-spacer /><v-btn text @click="showRecommendedReset = false"
            >Cancel</v-btn
          ><v-btn text color="primary" @click="resetRecommended"
            >Reset schedule</v-btn
          ></v-card-actions
        ></v-card
      >
    </v-dialog>
    <v-dialog v-model="showCounterReset" max-width="560">
      <v-card
        ><v-card-title class="error--text"
          >Reset all lifetime counters?</v-card-title
        ><v-card-text
          ><p>
            This cannot be undone from the UI. Type
            <strong>RESET LIFETIME COUNTERS</strong> to continue.
          </p>
          <v-text-field
            ...
            dense /></v-card-text
        ><v-card-actions
          ><v-spacer /><v-btn text @click="showCounterReset = false"
            >Cancel</v-btn
          ><v-btn
            text
            color="error"
...
export default class PageKlippertools extends Mixins(BaseMixin) {
  ...
  async exportData(): Promise<void> {
    const payload = await this.$socket.emitAndWait(
      "machine.klippertools.service.export",
      {},
    );
