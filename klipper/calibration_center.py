...
class CalibrationCenter:
    ...
    def _heater_options(self, eventtime):
        options = []
        available = self._object_status("heaters", eventtime).get(
            "available_heaters", []
        )
        for heater in available:
            min_temp, max_temp = self._heater_limits(heater)
            status = self._object_status(heater, eventtime)
            options.append({
                "name": heater,
                "min_temp": min_temp,
                "max_temp": max_temp,
                ...
                "control": self._settings().get(heater, {}).get("control"),
            })
        return options
    ...
    def _current_values(self, eventtime):
        ...
        return {
            ...
            "manual_probe": self._object_status("manual_probe", eventtime),
            "heaters": self._heater_options(eventtime),
    ...
    def get_status(self, eventtime):
        ...
        return {
            ...
            "printer_state": self._object_status("print_stats", eventtime).get("state"),
            "homed_axes": self._object_status("toolhead", eventtime).get("homed_axes", ""),
            "flows": flows,
